...
const AdForm = (props) => {
  ...
  const uploadImage = async () => {
    ...
    formData.append("upload_preset", "auctionIt");

    fetch("https://api.cloudinary.com/v1_1/doegyn8dt/image/upload/", {
      method: "post",
      body: formData,
    })
      .then((res) => res.json())
      .then((data) => {
        props.postAd({ ...form, image: data.secure_url })
      })
      .then(() => navigate("/"))
      .catch((err) => {
    ...
    //     "https://api.cloudinary.com/v1_1/adarsh-cloud/image/upload/",
    //     formData
    //   );
